...
FINAL_DISCORD_NEW_LISTINGS="${DISCORD_WEBHOOK_URL_NEW_LISTINGS:-}"
...
if [ -f "$SECRETS_FILE" ]; then
  ...
  [ -z "$FINAL_DISCORD_NEW_LISTINGS" ] && FINAL_DISCORD_NEW_LISTINGS="$(grep '^DISCORD_WEBHOOK_URL_NEW_LISTINGS=' "$SECRETS_FILE" 2>/dev/null | sed 's/^DISCORD_WEBHOOK_URL_NEW_LISTINGS=//' | head -1)" || true
...
if [ -n "${DISCORD_WEBHOOK_URL:-}" ] || [ -n "${DISCORD_WEBHOOK_URL_SALES:-}" ] || [ -n "${DISCORD_WEBHOOK_URL_REPRICES:-}" ] || [ -n "${DISCORD_WEBHOOK_URL_NEW_LISTINGS:-}" ] || [ -n "${DISCORD_WEBHOOK_URL_DB_EXPORT:-}" ] || [ -n "${DISCORD_WEBHOOK_URL_OPS:-}" ] || [ -n "${ADMIN_TOKEN:-}" ] || { [ -n "$FINAL_DISCORD" ] || [ -n "$FINAL_DISCORD_SALES" ] || [ -n "$FINAL_DISCORD_REPRICES" ] || [ -n "$FINAL_DISCORD_NEW_LISTINGS" ] || [ -n "$FINAL_DISCORD_DB_EXPORT" ] || [ -n "$FINAL_DISCORD_OPS" ] || [ -n "$FINAL_ADMIN" ]; }; then
  umask 077
  MERGE_TMP="$(mktemp)"
  {
    ...
    [ -n "$FINAL_DISCORD_NEW_LISTINGS" ] && printf '%s\n' "DISCORD_WEBHOOK_URL_NEW_LISTINGS=$FINAL_DISCORD_NEW_LISTINGS"
