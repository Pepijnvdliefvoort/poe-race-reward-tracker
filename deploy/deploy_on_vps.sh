...
if [ -n "${DISCORD_WEBHOOK_URL:-}" ] || [ -n "${DISCORD_WEBHOOK_URL_SALES:-}" ] || [ -n "${DISCORD_WEBHOOK_URL_REPRICES:-}" ] || [ -n "${DISCORD_WEBHOOK_URL_DB_EXPORT:-}" ] || [ -n "${ADMIN_TOKEN:-}" ] || { [ -n "$FINAL_DISCORD" ] || [ -n "$FINAL_DISCORD_SALES" ] || [ -n "$FINAL_DISCORD_REPRICES" ] || [ -n "$FINAL_DISCORD_DB_EXPORT" ] || [ -n "$FINAL_ADMIN" ]; }; then
  umask 077
  MERGE_TMP="$(mktemp)"
  {
