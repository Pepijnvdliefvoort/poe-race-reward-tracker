...
FINAL_DISCORD_BANS="${DISCORD_WEBHOOK_URL_BANS:-}"
...
if [ -f "$SECRETS_FILE" ]; then
  ...
  [ -z "$FINAL_DISCORD_BANS" ] && FINAL_DISCORD_BANS="$(grep '^DISCORD_WEBHOOK_URL_BANS=' "$SECRETS_FILE" 2>/dev/null | sed 's/^DISCORD_WEBHOOK_URL_BANS=//' | head -1)" || true
...
if [ -n "${DISCORD_WEBHOOK_URL:-}" ] || [ -n "${DISCORD_WEBHOOK_URL_SALES:-}" ] || [ -n "${DISCORD_WEBHOOK_URL_REPRICES:-}" ] || [ -n "${DISCORD_WEBHOOK_URL_NEW_ITEMS:-}" ] || [ -n "${DISCORD_WEBHOOK_URL_DB_EXPORT:-}" ] || [ -n "${DISCORD_WEBHOOK_URL_OPS:-}" ] || [ -n "${DISCORD_WEBHOOK_URL_BANS:-}" ] || [ -n "${DISCORD_WEBHOOK_URL_DAILY_SUMMARY:-}" ] || [ -n "${ADMIN_TOKEN:-}" ] || { [ -n "$FINAL_DISCORD" ] || [ -n "$FINAL_DISCORD_SALES" ] || [ -n "$FINAL_DISCORD_REPRICES" ] || [ -n "$FINAL_DISCORD_NEW_ITEMS" ] || [ -n "$FINAL_DISCORD_DB_EXPORT" ] || [ -n "$FINAL_DISCORD_OPS" ] || [ -n "$FINAL_DISCORD_BANS" ] || [ -n "$FINAL_DISCORD_DAILY_SUMMARY" ] || [ -n "$FINAL_ADMIN" ]; }; then
  umask 077
  MERGE_TMP="$(mktemp)"
  {
    ...
    [ -n "$FINAL_DISCORD_BANS" ] && printf '%s\n' "DISCORD_WEBHOOK_URL_BANS=$FINAL_DISCORD_BANS"
